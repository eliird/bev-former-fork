...
class TemporalSelfAttention(nn.Module):
    ...
    def forward(self,
        ...
        expected_num_value = (spatial_shapes[:, 0] * spatial_shapes[:, 1]).sum()
        assert expected_num_value == num_value
...
def test_temporal_self_attention():
    ...
    embed_dims = 256
    num_heads = 8
    num_levels = 1  # BEV temporal attention uses single level
    num_points = 4
    ...
    try:
        ...
        # Test inputs - temporal attention operates on BEV space
        batch_size = 2
        bev_h, bev_w = 50, 50
        ...
        # Forward pass with previous BEV
        # Temporal attention expects key/value to have shape (bs*num_bev_queue, num_queries, embed_dims)
        # Stack current query and previous BEV
        temporal_value = torch.stack([query, prev_bev], 1).reshape(batch_size * 2, num_queries, embed_dims)
        
        # Also expand reference points to match temporal batch size
        temporal_ref_points = reference_points.repeat(2, 1, 1, 1)  # (bs*2, num_queries, num_levels, 2)
        ...
        # When no previous BEV, the model will automatically create value from query
        # But we still need to expand reference points to match the internal temporal batch size
        with torch.no_grad():
            output_no_prev = model(
                query=query,
                key=None,  # Will auto-create from query
                value=None,  # Will auto-create from query
                reference_points=temporal_ref_points,  # Use same expanded reference points
                spatial_shapes=spatial_shapes,
                level_start_index=level_start_index
            )
        ...
        print("🎉 TemporalSelfAttention test PASSED!")
        return True
